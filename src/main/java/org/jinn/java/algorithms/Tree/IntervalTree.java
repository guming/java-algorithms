...
public class IntervalTree<O extends Object> {
    ...
    protected static class IntervalTreePrinter {
        ...
        private static <O extends Object> String getString(Interval<O> interval, String prefix, boolean isTail) {
            ...
            builder.append(prefix + (isTail ? "tail" : "notail ") + interval.toString() + "\n");
            ...
            if (children.size() > 0) {
                for (int i = 0; i < children.size() - 1; i++) {
                    builder.append(getString(children.get(i), prefix + (isTail ? "tail" : "notail"), false));
                }
                if (children.size() > 0) {
                    builder.append(getString(children.get(children.size() - 1), prefix + (isTail ? "tail" : "notail"),
                            true));
                }
            }
